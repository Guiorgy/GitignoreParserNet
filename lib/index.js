...
// console.log('compiled:', content, '==>', parsed);
  return {
    accepts: function (input, expected) {
      if (input[0] === '/') input = input.slice(1);
      input = '/' + input;
      if (expected != null && expected !== (negatives[0].test(input) || !positives[0].test(input))) {
console.log('accepts:', { input, expected, reNeg: negatives[0], Neg: negatives[0].test(input), rePos: positives[0], Pos: positives[0].test(input), "(Neg || !Pos)": (negatives[0].test(input) || !positives[0].test(input)) });
}
    ...
    denies: function (input, expected) {
      ...
      input = '/' + input;
      if (expected != null && expected !== (!(negatives[0].test(input) || !positives[0].test(input)))) {
console.log('denies:', { input, expected, reNeg: negatives[0], Neg: negatives[0].test(input), rePos: positives[0], Pos: positives[0].test(input), "!(Neg || !Pos)": !(negatives[0].test(input) || !positives[0].test(input)) });
}
      return !(negatives[0].test(input) || !positives[0].test(input));
    },
